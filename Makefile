...
export TERRAFORM_PROVIDER_VERSION ?= 3.12.5
...
xpkg.build.provider-upjet-harbor: do.build.images
...
uptest: $(UPTEST) $(KUBECTL) $(CHAINSAW) $(CROSSPLANE_CLI)
	...
	@KUBECTL=$(KUBECTL) HELM=$(HELM) CHAINSAW=$(CHAINSAW) CROSSPLANE_CLI=$(CROSSPLANE_CLI) CROSSPLANE_NAMESPACE=$(CROSSPLANE_NAMESPACE) $(UPTEST) e2e "${UPTEST_EXAMPLE_LIST}" --data-source="${UPTEST_DATASOURCE_PATH}" --setup-script=cluster/test/setup.sh --default-conditions="Test" || $(FAIL)
...
#   - Filters out non-Harbor YAML files by matching only the apiVersion: line against
#     harbor.*crossplane.io. Anchoring prevents false positives from description
...
example-lint: $(KUBECTL_VALIDATE)
	...
	find "$$tmpdir" -name "*.yaml" | while read f; do grep -q '^apiVersion:.*harbor.*crossplane\.io' "$$f" || rm -f "$$f"; done; \
